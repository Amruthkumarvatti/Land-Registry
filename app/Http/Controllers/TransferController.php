    ...
    public function destroy(Transfer $transfer)
    {
        if ($transfer->status === 'approved') {
            return redirect()->route('transfers.index')
                ->with('error', 'Approved transfers cannot be deleted.');
        }

        // delete transfer deed file if present
        if ($transfer->transfer_deed) {
            \Illuminate\Support\Facades\Storage::disk('public')->delete($transfer->transfer_deed);
        }
