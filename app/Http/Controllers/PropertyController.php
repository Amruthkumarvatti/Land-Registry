...
class PropertyController extends Controller
{
    ...
    {
        // Remove related records first to satisfy foreign key constraints
        // property_histories has FK -> properties.id and is not soft-deleted
        $property->history()->delete();

        // registrations use SoftDeletes; permanently remove them
        $property->registrations()->withTrashed()->get()->each(function ($r) {
            $r->forceDelete();
        });

        // transfers may reference property_id (hard delete)
        $property->transfers()->delete();
        ...
        // Permanently remove the property (force delete because model uses SoftDeletes)
    ...
    {
        $property->load(['currentOwner', 'transfers' => fn($q) => $q->where('status', 'approved')->latest()]);
