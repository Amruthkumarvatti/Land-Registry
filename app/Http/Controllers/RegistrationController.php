...
class RegistrationController extends Controller
{
    ...
    {
        // Remove associated transfer first to satisfy FK constraints
        if ($registration->transfer) {
            if ($registration->transfer->transfer_deed) {
                Storage::disk('public')->delete($registration->transfer->transfer_deed);
            }
            $registration->transfer->delete();
        }

        // Remove uploaded registration documents from storage then permanently delete record
        foreach (['deed_document', 'supporting_doc1', 'supporting_doc2'] as $file) {
    ...
    public function approve(Registration $registration)
    {
        ...
        abort_if($registration->status === 'approved', 400, 'Only pending or rejected registrations can be approved.');
