    ...
    protected static function boot()
    {
        ...
        static::creating(function ($model) {
            if (empty($model->survey_number)) {
                ...
                // Determine the highest numeric suffix for the year and increment it.
                // Use a raw table query so soft-deleted rows are considered (prevents reusing numbers).
                $table = $model->getTable();
                $max = DB::table($table)
                    ->where('survey_number', 'like', $prefix . '%')
                    ->selectRaw('MAX(CAST(SUBSTRING_INDEX(survey_number, "-", -1) AS UNSIGNED)) as max')
                    ->value('max');

                $next = ($max ? $max + 1 : 1);
                $model->survey_number = $prefix . str_pad($next, 6, '0', STR_PAD_LEFT);
            }
        });
    }
